...
export function validateJobOrder(data: unknown): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
...
export function applyDefaults(data: JobOrderData): JobOrderData {
  return {
    ...
    specialInstructions: data.specialInstructions?.trim() || DEFAULT_SPECIAL_INSTRUCTIONS,
