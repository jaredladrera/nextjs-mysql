...
const CSS = `
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .top-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 6px; }
  ...
  .doc-title { text-align: center; font-size: 16pt; font-weight: bold; letter-spacing: 3px; margin-bottom: 8px; }

  .vendor-bill-section { display: flex; border: 1px solid #000; }
  .vendor-box { flex: 1; padding: 5px 7px; border-right: 1px solid #000; font-size: 8.5pt; line-height: 1.6; }
  .bill-box   { flex: 1; padding: 5px 7px; font-size: 8.5pt; line-height: 1.6; }
  .field-label { font-weight: bold; }

  .items-table { width: 100%; border-collapse: collapse; border: 1px solid #000; border-top: none; font-size: 8.5pt; }
  .items-table thead tr { background-color: #000; color: #fff; }
  .items-table thead th { padding: 4px 5px; text-align: center; font-weight: bold; font-size: 8pt; border-right: 1px solid #555; }
  .items-table thead th:last-child { border-right: none; }
  .cell { padding: 3px 5px; border-right: 1px solid #000; border-bottom: 1px solid #ddd; vertical-align: top; }
  ...
  .remarks-section { flex: 1.5; padding: 5px 7px; border-right: 1px solid #000; font-size: 8.5pt; line-height: 1.5; }
  .totals-section  { flex: 1; font-size: 8.5pt; }
  .total-row { display: flex; justify-content: space-between; padding: 3px 8px; border-bottom: 1px solid #ccc; }
  ...
  .si-header { background-color: #000; color: #fff; font-weight: bold; padding: 3px 7px; font-size: 8.5pt; }
  ...
  .sig-box { flex: 1; border-right: 1px solid #000; padding: 5px 7px; font-size: 8pt; min-height: 55px; }
  .sig-box:last-child { border-right: none; }
  .sig-role  { font-weight: bold; background: #eee; padding: 2px 0; margin-bottom: 16px; font-size: 7.5pt; text-transform: uppercase; }
...
function buildTotals(totals: JobOrderTotals): string {
  return `
    <div class="total-row">
      <span>Total:</span><span>${formatCurrency(totals.subTotal)}</span>
    </div>
    <div class="total-row">
      <span>VAT ${totals.vatPercent}%</span><span>${formatCurrency(totals.vatAmount)}</span>
    </div>
    <div class="total-row bold" style="border-top:2px solid #000;">
      <span>JO Total</span><span>${formatCurrency(totals.joTotal)}</span>
    </div>`;
}

export function renderJobOrderHtml(data: JobOrderData, totals: JobOrderTotals): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
